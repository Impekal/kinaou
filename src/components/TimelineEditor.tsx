              ...
              return (
                <div className={track.locked ? 'clip lockedClip' : 'clip'} key={clip.id} style={{ marginLeft: `${Math.min(clip.startMs / 100, 140)}px`, width: `${Math.max(110, Math.min(clip.durationMs / 25, 240))}px` }}>
                  ...
                  <div className="clipActions">
                    ...
                    {(asset?.kind === 'video' || asset?.kind === 'audio') && <><button disabled={track.locked || clip.speed <= 0.25} onClick={() => apply({ type: 'set-clip-speed', trackId: track.id, clipId: clip.id, speed: Math.max(0.25, clip.speed / 2) })}>Speed ½</button><button disabled={track.locked || clip.speed >= 4 || !clipSpeedFitsSource(asset, clip, Math.min(4, clip.speed * 2))} title={clipSpeedFitsSource(asset, clip, Math.min(4, clip.speed * 2)) ? undefined : 'Doubling the speed would need more source media than this asset has'} onClick={() => apply({ type: 'set-clip-speed', trackId: track.id, clipId: clip.id, speed: Math.min(4, clip.speed * 2) })}>Speed 2×</button><button disabled={track.locked || clip.speed === 1} onClick={() => apply({ type: 'set-clip-speed', trackId: track.id, clipId: clip.id, speed: 1 })}>Speed reset</button></>}
