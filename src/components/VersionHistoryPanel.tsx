...
export function VersionHistoryPanel({ project, history, onProjectChange }: { project: KinaouProject; history: PersistentVersionHistory; onProjectChange: (project: KinaouProject) => void }) {
  ...
  const [message, setMessage] = useState<{ key: 'history.saved' | 'history.restored' | 'history.deleted'; label?: string } | null>(null)
  ...
  function restore(version: ProjectVersion) {
    ...
    try {
      ...
      setMessage({ key: 'history.restored', label: version.label }); refresh()
    } catch (cause) { fail(cause) }
  }

  function remove(version: ProjectVersion) {
    if (!window.confirm(t('history.confirmDelete', { label: version.label }))) return
    setError(''); setMessage(null)
  ...
  return <section className="card versionPanel">
    ...
    {message && <div className="note" role="status">{t(message.key, { label: message.label ?? '' })}</div>}
