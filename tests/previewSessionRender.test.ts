...
import { freshPreviewPlan, runTimelinePreview, type PreviewFeedback } from '../src/core/previewSession'
...
it('executes independent preview sessions through the real worker with exact frames, playback files and unchanged sources', async (context) => {
  ...
  const root = await mkdtemp(path.join(os.tmpdir(), 'kinaou-preview-session-'))
  const managed = path.join(root, 'KINAOU')
  await mkdir(path.join(managed, 'Assets'), { recursive: true })
  const child = spawn(process.execPath, [path.resolve('worker/mac-worker.mjs')], { env: { PATH: process.env.PATH, KINAOU_MANAGED_ROOT: managed, KINAOU_WORKER_TOKEN: 'preview-test', KINAOU_WORKER_PORT: '43938' }, stdio: ['ignore', 'pipe', 'pipe'] })
  let logs = ''
  ...
  try {
    ...
    while (!logs.includes('listening on http://127.0.0.1:43938')) {
    ...
    const client = new WorkerClient({ baseUrl: 'http://127.0.0.1:43938', token: 'preview-test' })
    const plans = [freshPreviewPlan(createTimelinePreviewPlan(project))]
    project.tracks[0].clips[0].sourceOffsetMs = 1000
    plans.push(freshPreviewPlan(createTimelinePreviewPlan(project)))
    const states: PreviewFeedback[][] = [[], []], blobs: Blob[] = []
    await Promise.all(plans.map((plan, index) => runTimelinePreview(plan, client, () => Date.now() < deadline + 20000, (state) => states[index].push(state), (blob) => { blobs[index] = blob }, undefined, () => new Promise((resolve) => setTimeout(resolve, 50)))))
    for (const [index, channel] of [[0, 0], [1, 2]]) {
      ...
      expect(probe.streams[0].width).toBe(960); expect(probe.streams[0].height).toBe(540)
