...
function trackChild(child) {
  // Subscribe immediately: cleanup must also work when an early exit already emitted close.
  const closed = new Promise((resolve) => child.once('close', resolve))
  return { closed, async stop() { child.kill('SIGKILL'); await closed } }
}

test('ducking cleanup handles a child that already closed', { timeout: 5000 }, async () => {
...
test('music is measurably quieter during a voice interval', { timeout: 300_000 }, async (t) => {
  ...
  } finally { await lifetime.stop(); await rm(root, { recursive: true, force: true }) }
})
