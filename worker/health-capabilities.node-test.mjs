...
test('health omits FFmpeg-backed capabilities when ffmpeg and ffprobe are missing', async () => {
  ...
  try {
    ...
    assert.ok(handshake.capabilities.includes('publish-package-integrity'))
...
test('health advertises FFmpeg-backed capabilities when ffmpeg and ffprobe respond', async () => {
  ...
  try {
    const handshake = await worker.health()
    for (const capability of ['filesystem', 'asset-upload', 'publish-package', 'publish-package-library', 'publish-package-integrity', 'publish-preflight', 'format-reframing', 'ffmpeg', 'media-probe', 'media-proxy', 'media-thumbnail', 'media-waveform']) {
