...
6. **Publish** lists only recorded successful exports. Enter destination/title/description/tags and click **Save as project defaults**; change the form, then **Use saved defaults**, and reopen the project once to confirm the saved values return automatically. **Clear saved defaults** must remove only the stored copy and keep the current form. Choose the new MP4 and create a local package. Expect a new uniquely named `*.publish.json` beside the MP4 in `KINAOU/Renders`; its media path/range/format must match the export and the MP4 must remain unchanged. Click **Refresh packages**: the new handoff appears with `MP4 AVAILABLE`; **Open metadata** restores its destination/title/description/tags and selects the matching recorded export. If the MP4 is later absent, refresh reports `MP4 MISSING` without deleting or changing anything. Nothing is sent to the selected platform.
