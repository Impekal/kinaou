...
Repo: `Impekal/kinaou`
Default branch: `main`
Current main SHA after PR #74: `7da21ebdbe0513b2cc84d5e0f58f79e74f4e91c8`
...
- Browser/app visuals now have both paths implemented and strictly separated: real capture through explicit user-initiated macOS screencapture jobs gated by the system Screen Recording permission (display/region screenshots and stoppable recordings; per-window capture is not implemented yet), and locally generated visuals carrying prompt/model/seed provenance. Generated visuals must never be presented as evidence of a real screen state, and captures always carry real-capture provenance.
...
The REAL CAPTURE path is now also merged (PR #71 worker jobs, PR #72 Capture Studio). Per-window targeting is covered by the PR #74 interactive selection. Remaining capture follow-ups when needed: a fixed-region picking UI and optional audio for recordings (a privacy decision for the user) — none blocking. The next milestone should come from the later roadmap (avatar, adapt/publish flow, commentary/reaction) together with the pending real-Mac execution test.
...
3. For real screen capture: grant macOS Screen Recording permission (System Settings → Privacy & Security → Screen Recording) to the terminal/process running the KINAOU worker when macOS prompts on the first capture.
